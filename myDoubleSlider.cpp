...
void myDoubleSlider::render(wxDC& dc)
{
	...
	DrawUpThumb(dc, SLIDER_MARGIN + leftslidervalue + 2, h*0.5 + 5);
	if (leftval < maxval / 2)
...
void myDoubleSlider::OnMotion(wxMouseEvent& event)
{
	...
	if (selectedslider != 0) {
		...
		pos.x = pos.x - SLIDER_MARGIN;
		pos.y = h - SLIDER_MARGIN - pos.y;
		//int m = prevx - pos.x; for a step size of 1
		if (prevx-pos.x < 0)
